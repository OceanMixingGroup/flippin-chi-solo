function correct_Jan_1_2019_dates_within_sat_file(sat_filename)
    ...
    % '+ffffffff????2a5c.......' % for an upcast and
    % '+ffff0000ffff0000????2a5c' for a surface file
    % where ????2a5c is the datestring, part of which is fixed by
    % the date, and part is variable with the slight time variability
    ...
    for line_no = 1:length(all_lines)
        ...
        if length(curr_line) < 25
            continue
        end
        if (strcmp(curr_line([1:9, 14:17]), '+ffffffff2a5c') | ...
            strcmp(curr_line([1:17, 22:25]), '+ffff0000ffff00002a5c'))
            ...
            if strcmp(curr_line(2:9), 'ffffffff')
                curr_line(1:17) = ['+ffffffff' new_t];
            elseif strcmp(curr_line(2:17), 'ffff0000ffff0000')
                curr_line(1:25) = ['+ffff0000ffff0000' new_t];
            end
